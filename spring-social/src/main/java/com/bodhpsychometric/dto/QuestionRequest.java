 ...
 * this field existed means), LINEAR_SCALE or LIKERT_GRID. On a LINEAR_SCALE
 * the options list is IGNORED: the backend generates the points 1—5 and
 * derives their MQT scores from mqtScores, so the only scale-specific fields a
 * caller sends are the two labels. On a LIKERT_GRID the options ARE the shared
