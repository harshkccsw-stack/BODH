...
public class ReportTagBinding implements java.io.Serializable {
    ...
    /**
     * VALUE only: which computation produces this tag's value.
     *
     * <p>A plain id and not a {@code @ManyToOne} — the same reason the schema
     * has no FK here. A template is portable across assessments while a
     * computation is bound to one, so an association would tie a template's
     * lifecycle to a computation it merely once referenced.
     */
    @Column(name = "reportComputationId")
    private Long reportComputationId;

    /**
     * VALUE only: which of that computation's outputs fills the tag.
     *
     * <p>In DIRECT mode this is the SLUG of a rule pinned into the computation;
     * in GENERATED mode it is a key the generated function declares. One column
     * for both is what lets a template bound today survive being regenerated by
     * a model later.
     */
    @Column(name = "outputKey", length = 80)
