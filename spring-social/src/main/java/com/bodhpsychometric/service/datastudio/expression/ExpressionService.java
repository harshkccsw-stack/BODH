...
/**
 * Parser and validator for the Data Studio formula language.
 *
 * <p>The grammar is a <b>closed whitelist</b>: arithmetic, comparisons,
 * boolean logic, and a fixed set of functions over dataset column keys. That
 * is the single most important property of this file. A formula is user text
 * arriving over HTTP, and it is never compiled to SQL, never reflected on and
 * never evaluated as code — it becomes an {@link Node} tree that
 * {@link ExpressionEvaluator} walks over row maps the server already holds.
 * An unknown identifier is a parse error, not an escape hatch.
 *
 * <p>Column references are <b>bracket-quoted</b> — {@code [mqt:14]},
 * {@code [demo:7]} — because real keys contain characters the grammar
 * otherwise owns ({@code -} is subtraction, {@code :} separates the family
 * prefix). The frontend's insert-column chips only ever emit the bracketed
 * form, so nobody types a raw key. Bare identifiers still parse for the
 * simple cases.
 *
 * <p>{@link #validate} also CLASSIFIES: a formula that only reads the current
 * row is CLIENT, one that calls a population function (ZSCORE, PERCENTILE,
 * RANK, AVERAGE, …) is SERVER, because answering it needs every row and the
 * browser is not guaranteed to hold them. That is a latency hint for the
 * grid, not a correctness switch — the sheet's data endpoint computes every
 * column server-side either way.
 */
@Service
public class ExpressionService {
     ...
     * a half-typed formula is a normal state, so problems come back in
     * {@link DsExprResponse#errors()} and the caller decides what to do.
     *
     * <p>An empty {@code availableColumns} means "do not check column names"
     * (nothing to check against), which is what the ad-hoc query path wants;
     * a sheet always passes its real column set, so a typo'd key is caught
     * before the column is ever saved.
     */
    public DsExprResponse validate(String expr, Set<String> availableColumns) {
        ...
        try {
            ...
            analyze(root, a);
            resultType = typeName(inferType(root));
        } catch (IllegalArgumentException ex) {
            a.errors.add(ex.getMessage());
        }
        return new DsExprResponse(
                a.errors.isEmpty(),
                a.serverUsed ? SERVER : CLIENT,
                resultType == null ? "number" : resultType,
                List.copyOf(a.errors),
                List.copyOf(a.columns),
                List.copyOf(a.functions));
    }
    ...
    private T inferType(Node n) {
        ...
        if (n instanceof Call) {
            Call c = (Call) n;
            if ("NORMBAND".equals(c.name)) return T.STRING;
            if ("AND".equals(c.name) || "OR".equals(c.name) || "NOT".equals(c.name)) return T.BOOLEAN;
            // IF is the one function whose result type is its ARGUMENTS' type,
            // not its own: IF(x > 3, "high", "low") is a banding label, and
            // declaring it numeric would have the grid render a text column as
            // a number. Either branch being text settles it.
            if ("IF".equals(c.name) && c.args.size() == 3) {
                T then = inferType(c.args.get(1));
                T otherwise = inferType(c.args.get(2));
