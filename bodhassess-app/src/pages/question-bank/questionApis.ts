 ...
 */
export type QuestionType = 'MCQ' | 'LINEAR_SCALE' | 'LIKERT_GRID';

/** The fixed width of a linear scale. Mirrors SCALE_FROM/SCALE_TO on the backend. */
export const SCALE_FROM = 1;
export const SCALE_TO = 5;
...
export const QUESTION_TYPES: Array<{ value: QuestionType; label: string; hint: string }> = [
  { value: 'MCQ', label: 'Multiple choice (MCQ)', hint: 'Options you write, each scored against MQTs.' },
  {
    value: 'LINEAR_SCALE',
    label: 'Linear scale',
    hint: `Points ${SCALE_FROM}—${SCALE_TO} with a label at each end. The point picked is the score, so only the question is mapped to MQTs.`,
  },
  {
    value: 'LIKERT_GRID',
...
export interface QuestionPayload {
   ...
   * On a LIKERT_GRID these are the shared COLUMNS.
   */
