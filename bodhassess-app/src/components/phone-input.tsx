...
/**
 * Country code + mobile number as ONE control. Same component as the portal's
 * (bodhassess-portal/src/components/phone-input.tsx) — separate packages, no
 * shared module, so change both together. They differ only in how the box
 * sizes itself, each matching the fields around it.
 *
...
/** Mirrors INPUT_CLASS — the class every other field on the dashboard uses —
 *  so this control sits at the same height and weight as its neighbours. The
 *  padding moves to the children, which is what lets the two halves share one
 *  border. */
const BOX =
  'flex w-full items-stretch rounded-lg border border-border bg-background text-sm ' +
...
export function PhoneInput({
  ...
  return (
    <div className={BOX}>
      <div className="relative flex shrink-0 items-center gap-1.5 py-2 pl-3 pr-2">
        {selected ? (
          <>
      ...
      <input
        ...
        disabled={disabled}
        title={title}
        className="w-full min-w-0 rounded-r-lg bg-transparent px-3 py-2 outline-none disabled:cursor-not-allowed"
      />
    </div>
  );
}
